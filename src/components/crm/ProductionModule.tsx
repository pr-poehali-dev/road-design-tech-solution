...
const SUBSECTIONS_II: SubSection[] = [
  { code: 'ИГДИ', name: 'Инженерно-геодезические изыскания', gostRef: 'СП 47.13330.2016, разд. 5', type: 'izisk' },
  { code: 'ИГИ', name: 'Инженерно-геологические изыскания', gostRef: 'СП 11-105-97, СП 47.13330.2016', type: 'izisk' },
  { code: 'ИЭИ', name: 'Инженерно-экологические изыскания', gostRef: 'СП 11-102-97, СанПиН 2.1.7.1287-03', type: 'izisk' },
  { code: 'ИГМИ', name: 'Инженерно-гидрометеорологические изыскания', gostRef: 'СП 131.13330.2020, СП 11-103-97', type: 'izisk' },
  { code: 'АРХ', name: 'Археологические изыскания', gostRef: 'ФЗ №73-ФЗ, Закон СПб №820-7', type: 'izisk' },
  { code: 'ОСС', name: 'Обследование существующих сооружений', gostRef: 'ГОСТ 31937-2011, СП 13-102-2003', type: 'izisk' },
  { code: 'ОТЧ', name: 'Технический отчёт сводный', gostRef: 'СП 47.13330.2016, ПП РФ №87', type: 'pz' },
];
  ...
  const buildContext = (section: SectionDef, subSection?: SubSection): string => {
    ...
    if (subSection) {
      ...
      const typeHints: Record<string, string> = {
        ...
        izisk: `Это ИНЖЕНЕРНЫЕ ИЗЫСКАНИЯ. Разработай ПОЛНОЦЕННЫЙ подраздел по нормативам:
ИГДИ → топосъёмка 1:500, расчёт реперов (≥4/км²), таблица объёмов (га/км/шт), SVG-схема реперов.
ИГИ → расчёт N скважин (S_га/0.75 для кат.II), глубина (формула СП 22.13330.2016), объём бурения пог.м, таблица ИГЭ (γ/E/φ/c/IL), SVG литоколонка + схема скважин.
ИЭИ → пробы почвы (≥5 точек/10 га × 3 глубины), пробы ГВ (≥3 шт), гамма-съёмка (S_м²/2500 точек), таблицы объёмов, SVG-схема точек опробования.
ИГМИ → таблица климат. параметров (СП 131.13330.2020): t_нар, снег, ветер, промерзание, осадки, уровни воды.
АРХ → кол-во шурфов (S_га/0.7), пешеход. маршруты, ГИКЭ, заключение КГИОП.
ОСС → инструмент. контроль (5 точек/50 м), вскрытия (≥2/объект), CCTV пог.м, категория I–IV по ГОСТ 31937-2011.
ОТЧ → все разделы + заключение о пригодности + рекомендации по фундаментам + итоговая таблица объёмов.
КРИТИЧНО: единицы — пог.м (бурение), шт (скважины/пробы), га (съёмка), км (ходы). НИКОГДА м³.`,
      };
