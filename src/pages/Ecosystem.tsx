    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
          ...
          >
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-cyan-400">Добро пожаловать в экосистему DEOD</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card className="bg-slate-800/50 border-cyan-500/20 p-6">
                <div className="flex items-center gap-3 mb-2">
                  <Icon name="TrendingUp" className="text-cyan-400" />
                  <p className="text-slate-400 text-sm">Личный оборот за квартал</p>
                </div>
                <p className="text-2xl font-bold text-white">{formatNumber(mockPartnerData.personalTurnover)} ₽</p>
              </Card>

              <Card className="bg-slate-800/50 border-blue-500/20 p-6">
                <div className="flex items-center gap-3 mb-2">
                  <Icon name="Network" className="text-blue-400" />
                  <p className="text-slate-400 text-sm">Оборот вашей сети</p>
                </div>
                <p className="text-2xl font-bold text-white">{formatNumber(mockPartnerData.networkTurnover)} ₽</p>
              </Card>

              <Card className="bg-slate-800/50 border-purple-500/20 p-6">
                <div className="flex items-center gap-3 mb-2">
                  <Icon name="Layers" className="text-purple-400" />
                  <p className="text-slate-400 text-sm">Глубина сети</p>
                </div>
                <p className="text-2xl font-bold text-white">{mockPartnerData.networkDepth} активных уровня</p>
              </Card>

              <Card className="bg-slate-800/50 border-pink-500/20 p-6">
                <div className="flex items-center gap-3 mb-2">
                  <Icon name="DollarSign" className="text-pink-400" />
                  <p className="text-slate-400 text-sm">Прогноз дохода за квартал</p>
                </div>
                <p className="text-2xl font-bold text-white">{formatNumber(mockPartnerData.quarterForecast)} ₽</p>
              </Card>
            </div>

            <Card className="mt-6 bg-slate-800/50 border-cyan-500/20 p-6">
              <div className="flex items-center justify-between mb-3">
                <p className="text-white font-semibold">До следующего грейда</p>
                <p className="text-cyan-400">{mockPartnerData.progressToNext.current}%</p>
              </div>
                    ...
                    <div className="p-6">
                      <div className="flex items-start gap-4">
                        <div className={`w-16 h-16 rounded-xl bg-gradient-to-br ${phase.color} flex items-center justify-center flex-shrink-0`}>
                          <Icon name={phase.icon as any} size={32} className="text-white" />
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-2">
                            <h3 className="text-2xl font-bold text-white">ФАЗА {phase.id}: {phase.title}</h3>
                            <Icon
                              name={selectedPhase === phase.id ? 'ChevronUp' : 'ChevronDown'}
                              className="text-cyan-400"
                            />
                          </div>
                          <p className="text-cyan-400 font-semibold mb-3">{phase.period}</p>
                          <p className="text-slate-300 mb-3">{phase.goal}</p>
                          <div className="flex items-center gap-2 text-green-400 font-semibold">
                            <Icon name="DollarSign" size={20} />
                            <span>{phase.income}</span>
                          </div>
                        </div>
                      </div>
                          ...
                          >
                            {phase.logic && (
                              <div className="mb-6 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                                <p className="text-blue-300 font-semibold mb-2">💡 Логика роста:</p>
                                <p className="text-slate-300">{phase.logic}</p>
                              </div>
                            )}

                            <div className="mb-6">
                              <p className="text-white font-semibold mb-3">📦 Материалы для работы:</p>
                            ...
                            {phase.task && (
                              <div className="p-4 bg-cyan-900/20 border border-cyan-500/30 rounded-lg">
                                <p className="text-cyan-300 font-semibold mb-2">🎯 Задание:</p>
                                <p className="text-white">{phase.task}</p>
                              </div>
                            )}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-slate-400 text-sm">Уровень {item.level}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-2xl font-bold text-cyan-400">{item.percent}%</p>
                        <p className="text-white">{formatNumber(item.amount)} ₽</p>
                      </div>
                    </div>
                  </motion.div>
                ))}

                <div className="p-6 bg-gradient-to-r from-cyan-900/40 to-blue-900/40 border-2 border-cyan-500 rounded-lg">
                  <div className="flex items-center justify-between">
                    <p className="text-xl font-bold text-white">Итого фонд распределения</p>
                    <p className="text-3xl font-bold text-cyan-400">{formatNumber(total)} ₽</p>
                  </div>
                </div>

                {simulatorGrade && (
                  <div className="p-6 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                    <p className="text-blue-300 font-semibold mb-2">💡 Ваш доход:</p>
            ...
            <Card className="bg-slate-800/50 border-cyan-500/20 p-8">
              <div className="space-y-6 mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-white font-semibold mb-3">Сумма сделки по КП</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="block text-white font-semibold mb-3">Финальная цена продажи</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="block text-white font-semibold mb-3">Уровень в сети</label>
                  <input
                    type="number"
                    min="1"
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="w-5 h-5"
                  />
                  <label htmlFor="firstDeal" className="text-white">
              ...
              <div className="space-y-4">
                <div className="p-4 bg-slate-700/30 rounded-lg">
                  <p className="text-slate-400 mb-2">Шаг 1: Доход по сетевой модели</p>
                  <p className="text-white">
                ...
                <div className="p-4 bg-slate-700/30 rounded-lg">
                  <p className="text-slate-400 mb-2">Шаг 2: Бонус за эффективность</p>
                  <p className="text-white">
                ...
                {income.startBonus > 0 && (
                  <div className="p-4 bg-cyan-900/20 border border-cyan-500/30 rounded-lg">
                    <p className="text-cyan-300 mb-2">Шаг 3: Стартовый бонус</p>
                    <p className="text-white">
                      Ваш личный % удвоен! +{' '}
                ...
                <div className="p-6 bg-gradient-to-r from-cyan-900/40 to-blue-900/40 border-2 border-cyan-500 rounded-lg">
                  <p className="text-xl font-bold text-white mb-2">ИТОГО С ЭТОЙ СДЕЛКИ:</p>
                  <p className="text-4xl font-bold text-cyan-400">{formatNumber(income.total)} ₽</p>
                </div>
              </div>
            </Card>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {phases.map((phase, index) => (
                <motion.div
                  ...
                  transition={{ delay: index * 0.1 }}
                >
                  <Card className="bg-slate-800/50 border-cyan-500/20 p-6 h-full">
                    <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${phase.color} flex items-center justify-center mb-4`}>
                      <Icon name={phase.icon as any} size={24} className="text-white" />
                    </div>
                    <h3 className="text-xl font-bold text-white mb-3">Фаза {phase.id}: {phase.title}</h3>
                    <ul className="space-y-2 mb-4">
                      {phase.materials.slice(0, 3).map((material, idx) => (
                        <li key={idx} className="flex items-start gap-2 text-slate-300">
                          <Icon name="FileText" size={16} className="text-cyan-400 flex-shrink-0 mt-1" />
      ...
      <section className="py-16 md:py-24">
        <div className="container mx-auto px-4">
          ...
          >
            <Card className="bg-gradient-to-br from-cyan-900/40 to-blue-900/40 border-cyan-500/30 p-12">
              <Icon name="Rocket" size={64} className="text-cyan-400 mx-auto mb-6" />
