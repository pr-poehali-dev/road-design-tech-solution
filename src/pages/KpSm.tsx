...
const ROADMAP = [
  { days: "1–3",   stage: "Анализ исходных данных, формирование структуры РД", result: "План работ утверждён" },
  { days: "4–8",   stage: "Расчёт электрических нагрузок (П1–П2.9, В1–В2.11, ККБ, К1–К6, серверная)", result: "Таблица нагрузок, схема принципиальная" },
  { days: "9–14",  stage: "Разработка плана силовых сетей (трассы кабелей, щиты, питание) с учётом высоты 6 м", result: "План силовых сетей" },
  { days: "15–20", stage: "Разработка плана освещения (расчёт по СанПиН, рабочее + аварийное)", result: "План освещения" },
  { days: "21–23", stage: "Схема заземления и уравнивания потенциалов (TN-C)", result: "Схема заземления" },
  { days: "24–26", stage: "Спецификация оборудования и ведомость объёмов работ", result: "Спецификация, ведомость" },
  ...
  { days: "30",    stage: "Корректировка по замечаниям, подписание акта выполненных работ", result: "Акт выполненных работ" },
];
...
export default function KpSm() {
  ...
  const handleExport = async () => {
    ...
    setExporting(true);
    try {
      // Загружаем оба изображения как base64 ДО рендера
      ...
      // Ждём перерисовку DOM с новыми src
      ...
      pdf.save(`КП_ЭОМ_${KP_NUM}_КапстройИнжиниринг.pdf`);
    } catch (e) {
      console.error(e);
    } finally {
        ...
        >
          ...
          <div className="bg-[#0d2158] px-10 py-7">
            <div className="flex items-start justify-between gap-6">
              ...
              <div className="text-right flex-shrink-0">
                <img src={logoB64} alt="Логотип" className="h-12 object-contain mb-2 ml-auto" />
          ...
          <div className="px-10 pb-8">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-gray-100">
                  <th className="border border-gray-300 px-3 py-1.5 text-left w-10">№</th>
                  <th className="border border-gray-300 px-3 py-1.5 text-left">Раздел</th>
                </tr>
              </thead>
              <tbody>
                {RD_SECTIONS.map((s) => (
                  <tr key={s.n} className={s.n % 2 === 0 ? "bg-gray-50" : ""}>
                    <td className="border border-gray-300 px-3 py-1.5 text-center font-semibold text-gray-600">{s.n}</td>
                    <td className="border border-gray-300 px-3 py-1.5">{s.title}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="mt-4 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-sm">
              <div className="font-semibold text-amber-800 mb-1">Дополнительно (не входит в стоимость)</div>
              <div className="text-gray-700">
                По данному разделу ЭОМ готовы осуществить:
                <span className="font-semibold text-amber-900"> поставку оборудования</span>,{" "}
                <span className="font-semibold text-amber-900">электромонтажные работы (ЭМ)</span>,{" "}
                <span className="font-semibold text-amber-900">пусконаладочные работы (ПНР)</span>.
                {" "}Стоимость определяется отдельно по запросу.
              </div>
            </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-gray-100">
                  <th className="border border-gray-300 px-3 py-1.5 text-left">Наименование</th>
                  <th className="border border-gray-300 px-3 py-1.5 text-right w-44">Сумма, ₽</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="border border-gray-300 px-3 py-1.5 font-semibold">Разработка РД ЭОМ (полный цикл)</td>
                  <td className="border border-gray-300 px-3 py-1.5 text-right font-bold text-blue-800 text-base">{fmt(TOTAL)}</td>
                </tr>
                <tr className="bg-gray-50 text-gray-500">
                  <td className="border border-gray-300 px-3 py-1.5 pl-6">в т.ч. НДС {VAT_RATE}%</td>
                  <td className="border border-gray-300 px-3 py-1.5 text-right">{fmt(VAT)}</td>
                </tr>
              </tbody>
            </table>
            <div className="mt-2 grid grid-cols-3 gap-2">
              {[
                "Фиксированная цена",
                "Корректировка по замечаниям — включена",
                "Выезд на объект не требуется",
              ].map((t) => (
                <div key={t} className="flex items-start gap-2 bg-green-50 border border-green-200 rounded-lg p-2 text-xs text-green-800">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-gray-100">
                  <th className="border border-gray-300 px-3 py-1.5 text-left">Платёж</th>
                  <th className="border border-gray-300 px-3 py-1.5 text-center w-16">%</th>
                  <th className="border border-gray-300 px-3 py-1.5 text-right w-36">Сумма, ₽</th>
                  <th className="border border-gray-300 px-3 py-1.5 text-left">Условие</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="border border-gray-300 px-3 py-1.5 font-semibold">Аванс</td>
                  <td className="border border-gray-300 px-3 py-1.5 text-center font-bold text-blue-700">70%</td>
                  <td className="border border-gray-300 px-3 py-1.5 text-right font-semibold">{fmt(ADV)}</td>
                  <td className="border border-gray-300 px-3 py-1.5 text-gray-600">после подписания договора</td>
                </tr>
                <tr className="bg-gray-50">
                  <td className="border border-gray-300 px-3 py-1.5 font-semibold">Окончательный платёж</td>
                  <td className="border border-gray-300 px-3 py-1.5 text-center font-bold text-blue-700">30%</td>
                  <td className="border border-gray-300 px-3 py-1.5 text-right font-semibold">{fmt(REST)}</td>
                  <td className="border border-gray-300 px-3 py-1.5 text-gray-600">после подписания акта выполненных работ</td>
                </tr>
              </tbody>
            </table>
            <div className="mt-1.5 text-sm text-gray-600 flex gap-6">
            ...
            <div className="mb-2 text-sm text-gray-600">
              Срок: <strong>Май 2026 г.</strong> · Отсчёт с момента заключения договора и получения аванса
            </div>
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-[#0d2158] text-white">
                  <th className="border border-blue-900 px-3 py-1.5 text-center w-20">День</th>
                  <th className="border border-blue-900 px-3 py-1.5 text-left">Этап</th>
                  <th className="border border-blue-900 px-3 py-1.5 text-left w-52">Результат</th>
                </tr>
              </thead>
              <tbody>
                {ROADMAP.map((r, i) => (
                  <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-blue-50"}>
                    <td className="border border-gray-300 px-3 py-1.5 text-center font-bold text-blue-800 whitespace-nowrap">{r.days}</td>
                    <td className="border border-gray-300 px-3 py-1.5 text-gray-700">{r.stage}</td>
                    <td className="border border-gray-300 px-3 py-1.5 text-gray-600 text-xs">{r.result}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="font-semibold text-sm text-green-700 mb-1.5">Что входит</div>
              ...
              <div>
                <div className="font-semibold text-sm text-red-600 mb-1.5">Что не входит</div>
                <div className="space-y-1">
                  {EXCLUDED.map((t, i) => (
                    <div key={i} className="flex items-start gap-2 text-sm text-gray-700">
            ...
            {/* Итог */}
            <div className="mt-6 bg-[#0d2158] rounded-xl px-8 py-5 text-white">
              <div className="text-base font-bold mb-3">Итого по коммерческому предложению</div>
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div className="bg-white/10 rounded-lg p-3">
                  <div className="text-xl font-bold">30 дней</div>
                  <div className="text-blue-200 text-xs mt-1">Срок выполнения</div>
                </div>
                <div className="bg-white/10 rounded-lg p-3">
                  <div className="text-xl font-bold">70% / 30%</div>
            ...
            {/* Реквизиты + Подпись на одном блоке */}
            <div ref={signatureRef} className="mt-6 border border-gray-200 rounded-lg overflow-hidden">
              {/* Реквизиты */}
              <div className="bg-gray-50 px-5 py-3 border-b border-gray-200">
                <div className="font-bold text-gray-800 text-xs uppercase tracking-wide mb-2">Реквизиты исполнителя</div>
                <div className="grid grid-cols-3 gap-x-6 gap-y-0.5 text-xs text-gray-600">
                  <div><span className="text-gray-400">Наименование:</span> ООО «КАПСТРОЙ-ИНЖИНИРИНГ»</div>
                  <div><span className="text-gray-400">ИНН:</span> 7814795454</div>
                  <div><span className="text-gray-400">КПП:</span> 781401001</div>
                  <div className="col-span-2"><span className="text-gray-400">Адрес:</span> 197341, г. Санкт-Петербург, Фермское шоссе, д. 12, лит. Ж, пом. 310-Н к3</div>
              ...
              <div className="px-5 py-4 flex items-end justify-between">
                <div>
                  <p className="text-xs text-gray-500 mb-1">С уважением,</p>
                  <p className="text-sm font-semibold text-gray-800">Генеральный директор ООО «КАПСТРОЙ-ИНЖИНИРИНГ»</p>
                  <p className="text-sm text-gray-700 mt-0.5">Шумов Иван Викторович</p>
                ...
                <div className="text-right">
                  <p className="text-xs text-gray-500 mb-1">Заказчик подтверждает:</p>
                  <p className="text-sm text-gray-600">Должность / ФИО</p>
                  <div className="mt-5 text-xs text-gray-400">
                    <div className="w-44 border-b border-gray-400 ml-auto" />
                    <div className="mt-1 text-right">(подпись / дата)</div>
                  </div>
                </div>
              </div>

              <div className="px-5 py-2 border-t border-gray-100 flex justify-between text-xs text-gray-400 bg-gray-50">
