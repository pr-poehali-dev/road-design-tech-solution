...
export default function SalesFunnel() {
  ...
  const getScoreMessage = () => {
    ...
    if (percentage === 100) return { text: 'Отлично! Вы полностью освоили воронку продаж!', color: 'text-emerald-400' };
    if (percentage >= 80) return { text: 'Хорошо! Вы понимаете основные принципы.', color: 'text-green-400' };
    if (percentage >= 60) return { text: 'Неплохо, но есть над чем поработать.', color: 'text-yellow-400' };
    return { text: 'Рекомендуем пройти обучение ещё раз.', color: 'text-orange-400' };
  };
