...
ПРАЙС-ЛИСТ компании (используй ТОЛЬКО эти позиции для расчёта):
{PRICE_LIST_STR}

СТРУКТУРА КП:
1. **Титульная страница**: Название КП, дата, клиент (если указан в ТЗ)
2. **Краткое резюме**: 3-5 предложений о сути проекта
3. **Состав работ**: Детальный перечень услуг по разделам, со ссылками на коды из прайса
4. **Расчёт стоимости**: Таблица с позициями: Код | Наименование | Ед. | Объём | Цена за ед. | Итого
5. **Итоговая стоимость**: Сумма по всем позициям с учётом минимальных заказов и спецправил
6. **Сроки реализации**: Ориентировочные сроки по этапам
7. **Условия работы**: Стандартные условия (предоплата 30%, сдача по актам)
8. **Действительность КП**: 30 дней с даты выпуска

ВАЖНО: Если в ТЗ не указаны объёмы — запроси или укажи "по факту обмеров". Применяй правила округления и минимальных заказов из прайса.
