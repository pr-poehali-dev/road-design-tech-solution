...
Составь коммерческое предложение на основе этих материалов."""
            ...
            for code, norms in NORMS_BY_SECTION.items():
                if code != 'default' and code in section_code_upper:
                    norms_extra += '\n' + norms
                    break
...
ИНЖЕНЕРНЫЕ ИЗЫСКАНИЯ (ИГДИ, ИГИ, ИЭИ):
→ Таблица физико-механических свойств грунтов (ИГЭ/горизонт/γ кН/м³/E МПа/φ°/с кПа/IL), литологическая колонка скважины в SVG, карта ИГЭ в SVG, рекомендации по фундаментам, гидрогеологический раздел (УГВ, агрессивность воды по ГОСТ 31383)
